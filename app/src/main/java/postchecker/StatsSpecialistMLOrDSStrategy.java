...
public class StatsSpecialistMLOrDSStrategy extends GenericProgramStrategy
    ...
    void setRequiredCourses() {
        ...
        requiredCourses.add(new CourseRequirement("CSCA48", retake_warning));
    }
    ...
    public void doSpecificChecks() {
        CourseList requirementsNoA48 = new CourseList();
        for (CourseRequirement course : requiredCourses.courses) {
            if (!course.hasCourseCode("CSCA48")) {
                requirementsNoA48.add(course);
            }
        }
        if (requirementsNoA48.average() < 2.5) {
            addFailure("You need an average of 2.5+ in " + requirementsNoA48);
        }
